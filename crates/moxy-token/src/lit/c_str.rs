...
/// A parsed Rust cstr literal token.
#[derive(Debug, Clone)]
...
impl std::hash::Hash for LitCStr {
...
impl Scan for LitCStr {
    fn scan(cursor: Cursor<'_>) -> Result<(Cursor<'_>, Self), LexError> {
        let end = scan_cooked(cursor, "c\"").or_else(|_| scan_raw(cursor, "cr"))?;
