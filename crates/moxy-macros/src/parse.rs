...
pub fn expand(input: &DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
                ...
                arms.push(quote! {
                    {
                        ...
                        let attempt = (|| -> Result<Self, ::moxy_token::parse::ParseError> {
                            #gate
                            #(#stmts)*
                            Ok(Self::#vname #ctor)
                        })();
    ...
    Ok(quote! {
        impl #imp ::moxy_token::Parse for #name #ty #wher {
...
fn value_expr(opts: &ParseOptions, ty: &Type, stream: &syn::Ident) -> proc_macro2::TokenStream {
    if let Some(path) = &opts.call {
        ...
        quote! { ::moxy_ast::Punctuated::parse_separated_nonempty(#stream)? }
    } else if opts.terminated {
        quote! { ::moxy_ast::Punctuated::parse_terminated(#stream)? }
